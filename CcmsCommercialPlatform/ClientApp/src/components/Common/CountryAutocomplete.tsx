...
// Comprehensive list of countries with ISO-3166-1 alpha-2 codes (Microsoft standard)
...
export interface CountryAutocompleteProps {
  ...
  onCountryOtherChange: (value: string) => void;
  countryError?: string;
  countryOtherError?: string;
  onErrorClear?: () => void;
}
...
export function CountryAutocomplete({
  ...
  // Update query when countryCode changes externally (e.g., from props)
  useEffect(() => {
    if (countryCode) {
  ...
  const filteredCountries = countryQuery
    ? COUNTRIES.filter(
        (country) =>
          ...
          country.code.toLowerCase().includes(countryQuery.toLowerCase())
      ).slice(0, 15)
  ...
  return (
    <>
      <div className="ct-autocomplete" ref={countryDropdownRef}>
        <input
          ref={countryInputRef}
          type="text"
          className={`ct-input--primary ${countryError ? 'ct-input--error' : ''}`}
          value={countryQuery}
        ...
        {isCountryDropdownOpen && filteredCountries.length > 0 && (
          <div className="ct-autocomplete__menu">
            {filteredCountries.map((country, index) => (
              <div
                key={country.code}
                className={`ct-autocomplete__item ${
                  index === highlightedCountryIndex ? 'ct-autocomplete__item--active' : ''
                }`}
                onClick={() => handleCountrySelect(country)}
                onMouseEnter={() => setHighlightedCountryIndex(index)}
              >
                {country.flag && <span className="ct-autocomplete__flag">{country.flag}</span>}
                <span className="ct-autocomplete__name">{country.name}</span>
                {country.code !== 'XX' && (
                  <span className="ct-autocomplete__code">{country.code}</span>
                )}
              </div>
            ))}
          </div>
        )}
        {countryError && <div className="ct-input-error">{countryError}</div>}
      </div>
      {countryCode === 'XX' && (
        <div className="ct-input-group" style={{ marginTop: '0.5rem' }}>
          <label className="ct-label--primary">
            Country (Other) <span className="ct-required">*</span>
          </label>
          <input
            type="text"
            className={`ct-input--primary ${countryOtherError ? 'ct-input--error' : ''}`}
            value={countryOther}
            onChange={(e) => {
              onCountryOtherChange(e.target.value);
              if (onErrorClear) {
                onErrorClear();
              }
            }}
          ...
          />
          {countryOtherError && <div className="ct-input-error">{countryOtherError}</div>}
        </div>
      )}
    </>
  );
